...
class TestActorIntegration:
    ...
    def test_actor_creation_with_permissions(self):
        ...
        patient = Actor(
            name="John Doe", role=ActorRole.PATIENT, organization="Test Hospital"
        )
    ...
    def test_permission_manager_integration(self):
        ...
        assert not patient_perms.has_permission(
            "read:patient"
        )  # Can't read other patients
    ...
    def test_session_management_integration(self):
        ...
        actor = Actor(
            name="Test User", role=ActorRole.NURSE, organization="Test Hospital"
        )

        # Create session
        session = session_manager.create_session(
            user_id="test_user", actor=actor, organization="Test Hospital"
        )
...
class TestMCPToolIntegration:
    ...
    async def test_mcp_tool_with_insufficient_permissions(self):
        ...
        actor = Actor(
            name="Test Patient", role=ActorRole.PATIENT, organization="Test Hospital"
        )
        ...
        with patch("hacs_utils.mcp.tools.HACS_TOOLS_AVAILABLE", True):
            result = await execute_hacs_tool("create_resource", params, actor=actor)
...
class TestEndToEndWorkflow:
    ...
    async def test_complete_healthcare_workflow(self):
        ...
        # 2. Start session
        session_manager = SessionManager()
        session = session_manager.create_session(user_id="emily_chen", actor=physician)
        ...
        with patch("hacs_utils.mcp.tools.HACS_TOOLS_AVAILABLE", True):
            with patch("hacs_utils.mcp.tools._get_tool_function") as mock_get_tool:
                mock_tool = AsyncMock(
                    return_value={"success": True, "patient_id": "patient_456"}
                )
                mock_get_tool.return_value = mock_tool
