...
def test_debug_warnings_and_performance():
    ...
    with warnings.catch_warnings(record=True) as warning_list:
        ...
        from hacs_models import get_model_registry, Patient, Observation, MedicationRequest
    ...
    print(f"   100 registry calls: {registry_time:.3f}s ({registry_time*10:.1f}ms per call)")
    ...
    for model in test_models:
        start_time = time.time()
        for _ in range(10):
            result = schema_discovery.get_hacs_resource_schema(model, include_examples=False)
        end_time = time.time()
    ...
    print(f"   50 simple patients: {simple_creation_time:.3f}s ({simple_creation_time*20:.1f}ms per creation)")
    ...
    print(f"   25 complex observations: {complex_creation_time:.3f}s ({complex_creation_time*40:.1f}ms per creation)")
    ...
    print(f"   Success rate: {successful_creates}/20 ({successful_creates/20*100:.1f}%)")
    ...
    print(f"   Schema discovery: {sum(discovery_times.values())/len(discovery_times)*1000:.1f}ms avg")
    ...
    print(f"   Memory overhead: {after_creation_objects - initial_objects:,} objects per 100 resources")
