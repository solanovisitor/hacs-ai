...
# Core HACS imports
try:
    from hacs_persistence import HACSConnectionFactory, get_default_adapter, ensure_database_ready
...
from hacs_models import Actor, MemoryBlock, Patient
...
class MockVectorStore:
    ...
    def add_vectors(self, ids, vectors, metadata_list):
        ...
        for i, (vector_id, vector, metadata) in enumerate(zip(ids, vectors, metadata_list)):
...
class MockDBAdapter:
    ...
    def create_resource(self, resource):
        ...
        resource_data = resource.model_dump() if hasattr(resource, 'model_dump') else resource
        resource_id = resource_data.get("id", f"mock-{len(self.resources)}")
        self.resources[resource_id] = resource_data
        return resource_id
...
class TestPersistenceIntegration:
    ...
    @pytest.mark.skipif(not PERSISTENCE_AVAILABLE, reason="hacs-persistence not available")
    ...
    def test_connection_factory_adapter_caching(self):
        ...
        with patch.dict(os.environ, {"DATABASE_URL": "postgresql://test:test@localhost/test"}):
            with patch('hacs_persistence.connection_factory.create_postgres_adapter') as mock_create:
                mock_adapter = MagicMock()
                mock_create.return_value = mock_adapter
                
                # Mock migration status and run_migration
                with patch('hacs_persistence.connection_factory.get_migration_status') as mock_status:
                    with patch('hacs_persistence.connection_factory.run_migration') as mock_migration:
    ...
    def test_memory_tool_persistence_integration(self, mock_db_adapter, mock_vector_store):
...
class TestSecurityIntegration:
    ...
    def test_data_access_permission_validation(self, test_actor):
        ...
        result = security_context.validate_data_access_permissions("Observation", "write")
        assert result is True
    
    @pytest.mark.asyncio
    async def test_tool_execution_with_security_integration(self, mock_db_adapter, test_actor):
...
class TestEndToEndIntegration:
    ...
    @pytest.mark.asyncio
    async def test_complete_memory_workflow(self, mock_db_adapter, mock_vector_store, test_actor):
