...
def test_comprehensive_model_validation():
    ...
    try:
        ...
        for resource_name in test_models:
            ...
            result = schema_discovery.get_hacs_resource_schema(
                resource_name, include_examples=True, include_validation_rules=True
            )
            ...
            if not result.success:
                debug_print(
                    f"Schema discovery failed for {resource_name}: {result.message}"
                )
    ...
    try:
        ...
        for test_case in test_cases:
            ...
            try:
                ...
                debug_print(
                    f"Create result for {test_case['name']}: {create_result.success}"
                )
                ...
                assert create_result.success, (
                    f"Failed to create {test_case['name']}: {create_result.message}"
                )
    ...
    debug_print(
        "Testing integration between schema discovery and resource management..."
    )
    try:
        # Get schema for Patient and create a resource based on it
        patient_schema = schema_discovery.get_hacs_resource_schema(
            "Patient", include_examples=True
        )
