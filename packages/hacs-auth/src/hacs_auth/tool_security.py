...
from .actor import Actor, ActorRole, PermissionLevel 
...
class ToolSecurityContext:
    ...
    def __init__(
        ...
        elif dev_mode and not self.actor.has_active_session():
    ...
    def log_tool_execution(
        ...
        audit_event = AuditEvent(
            ...
            details={
                ...
                "actor_role": self.actor.role.value if hasattr(self.actor.role, 'value') else str(self.actor.role),
                "error": error
            },
...
def create_secure_actor(
    ...
    logger.info(f"Created secure actor {actor_name} with role {role} and {len(permissions)} permissions")
