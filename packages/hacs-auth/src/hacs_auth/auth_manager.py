...
class AuthConfig(BaseModel):
    ...
    oauth2_redirect_uri: str | None = Field(None, description="OAuth2 redirect URI")
    ...
    allowed_algorithms: list[str] = Field(
        default_factory=lambda: ["HS256", "RS256"], description="Allowed JWT algorithms"
    )
...
class TokenData(BaseModel):
    ...
    not_before: datetime | None = Field(None, description="Token not valid before")

    # Healthcare-specific claims
    organization: str | None = Field(None, description="Healthcare organization")
    department: str | None = Field(None, description="Department within organization")
    security_level: str = Field(default="medium", description="Security clearance level")
    session_id: str | None = Field(None, description="Associated session identifier")
    ...
    subject: str | None = Field(None, description="Token subject (usually user_id)")
...
class AuthManager:
    ...
    def _create_default_config(self) -> AuthConfig:
        ...
        return AuthConfig(
            ...
            token_expire_minutes=int(
                os.getenv("HACS_TOKEN_EXPIRE_MINUTES", "15")
            ...
            require_mfa=os.getenv("HACS_REQUIRE_MFA", "true").lower()
            == "true",  # Changed default to true
            max_session_duration_hours=int(
                os.getenv("HACS_MAX_SESSION_HOURS", "4")
    ...
    def verify_token(self, token: str) -> TokenData:
        ...
        try:
            ...
            issued_at = (
                datetime.fromtimestamp(iat, tz=UTC) if iat else datetime.now(UTC)
            )
            expires_at = (
                datetime.fromtimestamp(exp, tz=UTC) if exp else datetime.now(UTC)
            )
            not_before = datetime.fromtimestamp(nbf, tz=UTC) if nbf else None
