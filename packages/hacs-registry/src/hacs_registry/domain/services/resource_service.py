...
class ResourceService(IResourceService):
    ...
    async def register_resource(
        ...
        try:
            ...
            if self.validation_service:
                validation_errors = (
                    await self.validation_service.validate_resource_creation(
                        resource, actor_id
                    )
                )
                if validation_errors:
                    raise ValidationException(
                        f"Resource validation failed: {validation_errors}"
                    )
            ...
            self.logger.info(
                f"Resource registered: {resource_id} ({hacs_resource_class})"
            )
    ...
    async def update_resource(
        ...
        try:
            ...
            if self.validation_service:
                validation_errors = (
                    await self.validation_service.validate_resource_update(
                        resource, updates, actor_id
                    )
                )
                if validation_errors:
                    raise ValidationException(
                        f"Resource update validation failed: {validation_errors}"
                    )
    ...
    async def find_resources_by_category(
    ...
    async def find_resources_by_status(
...
class ResourceValidationService:
    ...
    async def validate_resource_creation(
        self, resource: ResourceAggregate, actor_id: Optional[str] = None
    ) -> List[str]:
    ...
    async def validate_resource_update(
        ...
        if "status" in updates:
            if not resource._is_valid_status_transition(
                resource.status, updates["status"]
            ):
                errors.append(
                    f"Invalid status transition from {resource.status} to {updates['status']}"
                )
...
class ResourceLifecycleService:
    ...
    async def change_status(
        ...
        self.logger.info(
            f"Resource {resource.id} status changed: {old_status} -> {new_status}"
        )
