...
from .migrations import get_migration_status, run_migration
...
class HACSConnectionFactory:
    ...
    def get_adapter(
        ...
        try:
            ...
            adapter = PostgreSQLAdapter(
                database_url=database_url, schema_name=schema_name, pool_size=pool_size
            )
    ...
    @classmethod
    def create_test_adapter(cls, test_db_url: str | None = None) -> PostgreSQLAdapter:
        """
        ...
        if not test_db_url:
            test_db_url = os.getenv(
                "TEST_DATABASE_URL", "postgresql://test:test@localhost/hacs_test"
            )

        return cls.get_adapter(
            database_url=test_db_url, schema_name="test_hacs", auto_migrate=True, pool_size=5
        )
    ...
    @classmethod
    @asynccontextmanager
    async def get_connection(cls, database_url: str | None = None):
        """
...
async def ensure_database_ready(database_url: str | None = None) -> bool:
    """
