...
def search_umls(term_or_code: str, *, version: str = "current", pageSize: int = 5) -> HACSResult:
    ...
    try:
        ...
        return HACSResult(success=True, message=f"Found {len(results)} matches", data={"results": results})
...
def get_umls_cui(cui: str, *, version: str = "current") -> HACSResult:
    ...
    try:
        ...
        if not res.get("success"):
            return HACSResult(success=False, message="UMLS CUI lookup failed", error=res.get("error"))
...
def crosswalk_umls(source: str, code: str, *, version: str = "current") -> HACSResult:
    ...
    try:
        ...
        if not res.get("success"):
            return HACSResult(success=False, message="UMLS crosswalk failed", error=res.get("error"))
...
def suggest_resource_codings(resource: Dict[str, Any], *, code_fields: Optional[List[str]] = None) -> HACSResult:
    ...
    return HACSResult(success=True, message=f"Found {len(suggestions)} coding candidates", data={"candidates": suggestions})


def summarize_codable_concepts(resource: Dict[str, Any], *, query: Optional[str] = None, top_k: int = 3) -> HACSResult:
    """
    ...
    return HACSResult(success=True, message="Summary generated", data={"summary": summary_text, "candidates": candidates, "umls": umls_items})

def get_possible_codes(resource: Dict[str, Any], *, query: Optional[str] = None, top_k: int = 3) -> HACSResult:
    """
...
def map_terminology(resource: Dict[str, Any], source: str, target: str, *, top_k: int = 3) -> HACSResult:
    ...
    for c in candidates:
        if (c.get('system') or '').lower().find(source.lower()) >= 0:
            ...
            if out:
                mappings.append({'field': c['field'], 'source': {'system': c['system'], 'code': code}, 'targets': out[:top_k]})

    return HACSResult(success=True, message=f"Computed {len(mappings)} mappings", data={'mappings': mappings})
