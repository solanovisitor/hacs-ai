...
class MedicationStatement(DomainResource):
    ...
    status: str = Field(default="active", description="active | completed | intended | stopped | on-hold | unknown | not-taken")
    medication_text: str | None = Field(default=None, description="Free text medication description")
    subject_ref: str | None = Field(default=None, description="Reference to Patient or Group")
    effective_datetime: str | None = Field(default=None, description="When the medication is/was taken (ISO 8601)")
