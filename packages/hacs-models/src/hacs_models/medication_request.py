...
class Dosage(DomainResource):
    ...
    sequence: int | None = Field(
        default=None,
        description="Order of administration instructions",
        examples=[1, 2, 3],
        ge=1
    )
    ...
    site: CodeableConcept | None = Field(
        default=None,
        description="Body site to administer medication"
    )
    
    # How drug should enter body
    route: CodeableConcept | None = Field(
        default=None,
        description="How drug should enter body (oral, injection, topical, etc.)"
    )
    ...
    max_dose_per_lifetime: dict[str, Any] | None = Field(
        default=None,
        description="Upper limit on medication per lifetime of patient"
    )
...
class MedicationRequestSubstitution(DomainResource):
    ...
    resource_type: Literal["MedicationRequestSubstitution"] = Field(default="MedicationRequestSubstitution")
    ...
    allowed_codeable_concept: CodeableConcept | None = Field(
        default=None,
        description="Whether substitution is allowed (coded)"
    )
...
class MedicationRequest(DomainResource):
    ...
    do_not_perform: bool | None = Field(
        default=None,
        description="True if medication must not be performed"
    )
    
    # Medication to be taken - REQUIRED
    medication_codeable_concept: CodeableConcept | None = Field(
        default=None,
        description="Medication to be taken (coded)"
    )
    ...
    reason_code: list[CodeableConcept] = Field(
        default_factory=list,
        description="Reason or indication for ordering the medication"
    )
    ...
    def get_medication_display(self) -> str:
        ...
        if self.medication_codeable_concept:
            if hasattr(self.medication_codeable_concept, 'text') and self.medication_codeable_concept.text:
                return self.medication_codeable_concept.text
            elif hasattr(self.medication_codeable_concept, 'coding') and self.medication_codeable_concept.coding:
    ...
    def add_dosage_instruction(
        ...
        dosage = Dosage(
            text=text,
            sequence=len(self.dosage_instruction) + 1
        )
    ...
    def add_reason_code(self, code: str, display: str, system: str = "http://snomed.info/sct") -> CodeableConcept:
