...
class ToolDefinition(BaseResource):
    ...
    function: Callable | None = Field(default=None, description="Tool function")

    # Optional args schema (Pydantic) for integrations/LLMs
    args_schema: dict[str, Any] | None = Field(
        default=None, description="JSON schema for tool inputs if available"
    )

    # Tool metadata fields
    module_path: str | None = Field(
        default=None, description="Module path where tool is defined"
    )
    function_name: str | None = Field(default=None, description="Function name")
    category: str | None = Field(default="general", description="Tool category")
    domain: str | None = Field(default="general", description="Tool domain")
    tags: list[str] = Field(default_factory=list, description="Tool tags")

    # Tool requirements
    requires_actor: bool = Field(
        default=False, description="Whether tool requires actor authentication"
    )
    requires_db: bool = Field(default=False, description="Whether tool requires database")
    requires_vector_store: bool = Field(
        default=False, description="Whether tool requires vector store"
    )
    is_async: bool = Field(default=False, description="Whether tool is async")

    # Tool status
    status: str | None = Field(default="published", description="Tool status")
